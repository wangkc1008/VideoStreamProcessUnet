...
def save_pic(pic, save_name="fig", save_pic=0, is_heat=False):
    ...
    pic = pic / np.max(pic) * 255
    pic = np.array(pic, np.uint8)
    if(is_heat):
        pic = cv2.cvtColor(pic, cv2.COLOR_GRAY2RGB)
        pic = cv2.applyColorMap(pic, 2)

    cv2.imwrite('/home/wzq/PycharmProjects/HT_Media/Classification/feature_maps/' + save_name + ".png", pic)